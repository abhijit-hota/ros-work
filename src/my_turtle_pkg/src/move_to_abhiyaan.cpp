#include <ros/ros.h>
#include <geometry_msgs/Twist.h>
#include <turtlesim/Pose.h>
#include <iostream>
#include <cmath>
#include <ros/topic_manager.h>

turtlesim::Pose turtlesim_pose;
turtlesim::Pose target_pose;

ros::Subscriber pose_subscriber;
ros::Subscriber destination_sub;

ros::Publisher velocity_publisher;

geometry_msgs::Twist vel_msg;

double getDistance(double x1, double y1, double x2, double y2){
    return sqrt(pow((x2 - x1), 2) + pow((y2 - y1), 2));
}

void poseCallback(const turtlesim::Pose::ConstPtr &pose_message){
    turtlesim_pose.x = pose_message->x;
    turtlesim_pose.y = pose_message->y;
    turtlesim_pose.theta = pose_message->theta;
}

void desCallback(const turtlesim::Pose::ConstPtr &tar_message){
    target_pose.x = tar_message->x;
    target_pose.y = tar_message->y;
}

void reachAbhiyaan(){
    ros::Rate loop_rate(30);

    double target_theta = atan2((target_pose.y - turtlesim_pose.y),
                                (target_pose.x - turtlesim_pose.x));

    if (target_theta<0) target_theta += 2 * M_PI;

    if (abs(target_theta - turtlesim_pose.theta) < 0.1){
        vel_msg.linear.x = 10;
        vel_msg.angular.z = 0;
    } else {
        vel_msg.linear.x = 0;
        vel_msg.angular.z = 5;
    }
    velocity_publisher.publish(vel_msg);
    loop_rate.sleep();
}

void reachedAbhiyaan() {

    vel_msg.linear.x = 0;
    vel_msg.angular.z = 0;
    velocity_publisher.publish(vel_msg);

    std::cout << "\n=====================" << std::endl;
    std::cout << "\n Found Abhiyaan! \n";

    ros::shutdown();
}

void go(){

    ros::Rate loop_rate(30);
    double distance;

    while(ros::ok()){

        distance = getDistance(turtlesim_pose.x, turtlesim_pose.y, target_pose.x, target_pose.y);

        bool flag = (distance == 0) ? false : true;

        if(distance < 2 && flag) reachedAbhiyaan();
        else {
            ROS_INFO("%lf units to reach Abhiyaan", distance);
            reachAbhiyaan();
        }

        loop_rate.sleep();
        ros::spinOnce();
    }

}

int main(int argc, char **argv){

    ros::init(argc, argv, "move_to_abhiyaan");
    ros::NodeHandle nh;

    pose_subscriber = nh.subscribe("/turtle1/pose", 1, poseCallback);
    destination_sub = nh.subscribe("/abhiyaan/pose" , 1 , desCallback);
    velocity_publisher = nh.advertise<geometry_msgs::Twist>("/turtle1/cmd_vel", 10);

    // ros::TopicManager topic_man;
    // size_t ncd = topic_man.getNumSubscriptions();
    // while(topic_man.getNumSubscriptions() < 1 && topic_man.getNumSubscribers("/turtle1/pose") < 1)
    //     ROS_INFO("%d",topic_man.getNumSubscriptions());
    //     ROS_INFO("%d",topic_man.getNumSubscribers("/turtle1/pose"));
    //     loop_rate.sleep();

    go();

    return 0;
}